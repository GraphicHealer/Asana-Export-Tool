...
class AsanaExport:
    ...
    def scan_teams(self):
        ...
        for i in range(len(self.teams)):
            ...
            id = 'g_'+str(self.teams[i].gid)
            self.logger(self.teams[i].name, parent='Teams', id=id)
            self.logger(self.teams[i].raw, parent=id)
            self.projects.extend(self.API.getProjects(self.teams[i], logger=self.logger, parent=id))
    ...
    def scan_projects(self):
        ...
        for i in range(len(self.projects)):
            if self.status == 'Cancel':
                ...
                break
            self.total_progress.set(i+1)
            id = 'p_'+str(self.projects[i].gid)
            self.logger(self.projects[i].name, parent='Projects', id=id)
            self.logger(self.projects[i].raw, parent=id)
            self.attachments.extend(self.API.getAttachments(self.projects[i], logger=self.logger, parent=id))
            self.tasks.extend(self.API.getTasks(self.projects[i], logger=self.logger, parent=id))
    ...
    def scan_tasks(self):
        ...
        for i in range(len(self.tasks)):
            if self.status == 'Cancel':
                ...
                break
            self.total_progress.set(i+1)
            id = 't_'+str(self.tasks[i].gid)
            self.logger(self.tasks[i].name, parent='Tasks', id=id)
            self.logger(self.tasks[i].raw, parent=id)
            self.attachments.extend(self.API.getAttachments(self.tasks[i], logger=self.logger, parent=id))
    ...
    def parse_attachments(self):
        ...
        for i in range(len(self.attachments)):
            ...
            if self.status == 'Cancel':
                ...
                break
            self.total_progress.set(i+1)
            id = 'a_'+str(self.attachments[i].gid)
            self.logger(self.attachments[i].name, parent='Attachments', id=id)
            self.logger(self.attachments[i].raw, parent=id)
    ...
    def download_attachments(self):
        ...
        for i in range(len(self.attachments)):
            if self.status == 'Cancel':
                ...
                break
            self.total_progress.set(i+1)
            id = 'd_'+str(self.attachments[i].gid)
            self.logger(self.attachments[i].name, parent='Downloads', id=id)
            self.logger(self.attachments[i].raw, parent=id)
            self.download_file(self.API.getAttachmentURL(self.attachments[i], logger=self.logger, parent=id), self.attachments[i].path, parent=id)
